        ...
        with open(manifest_file, 'r') as f:
            ...
            for i, row in enumerate(reader, 1):
                ...
                for col in required_columns[1:]:  # Skip sampleId, already checked
                    ...
                    if col == 'lr_reads':
                        if not any(value.lower().endswith(ext) for ext in ['.fasta', '.fna', '.fa', '.fas']):
                            print(f"WARNING: Row {i} (sample {sample_id}): lr_reads file doesn't appear to be FASTA format (expected: .fasta, .fna, .fa, .fas)")
